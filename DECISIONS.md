...
## 2026-07-19 — Installed skills: CLAUDE.md wins on conflicts; dotnet-best-practices removed
Context: Three Claude Code skills were installed from public repos (`blazor-expert`, `dotnet-best-practices`, `improve-codebase-architecture`). `dotnet-best-practices` contained rules that directly contradict this project's established conventions: mandatory XML doc comments (vs §4/§9 "no comments that restate code"), MSTest + FluentAssertions + Moq (vs xUnit + in-memory SQLite, no mocks), ResourceManager localization (vs hard-coded const messages), primary constructor DI syntax (not used here), and ConfigureAwait(false) (irrelevant in ASP.NET Core).
Decision: Uninstall `dotnet-best-practices`. Kept: `blazor-expert` (reference material, no rule conflicts), `improve-codebase-architecture` (user-invoked architectural review, `disable-model-invocation: true`). General rule: when any installed skill contradicts `CLAUDE.md` or this file, the project files win — skills are advisory, not authoritative.
Consequences: Agents and users invoking `/dotnet-best-practices` will get an error (skill not found), which is the correct outcome. If a useful rule from that skill is ever needed, it should be explicitly adopted into `CLAUDE.md`.
